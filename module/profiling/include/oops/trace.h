...
struct Location { // 打点位置相关
  Location() = default;
  Location(const char *label, const char *file, int line)
...
private:
  ...
  TimeInterval GetRecordTableImpl(int node_i, size_t depth,
  ...
  std::stack<TimePoint>
      scope_stack_; // 维护当前Scope前一个TimePoint，每次Trace，读取并刷新TimePoint
};

class TraceScopeBase {
protected:
  void ThrowCountMismatchException(unsigned char scope_count,
                                   unsigned char trace_count, const char *label,
                                   const char *file, int line);
};

// Scope为粒度，每层统计作用域
template <typename> class TraceScope : public TraceScopeBase {
public:
  ...
  template <typename>
  void TraceImpl(unsigned char count, const char *label, const char *file,
                 int line) {
    if (count != count_) {
  ...
  void Trace(
      ...
      size_t
          mask) { // 做成类成员函数以便在编译器找到部分SCOPE和TRACE不匹配的问题
  ...
  template <typename Lambda>
  void Trace(Lambda &&, const char *label, const char *file, int line,
...
template <typename Lambda> auto MakeTraceScope(Lambda &&) {
