...
#define GET(member)                                                            \
  [](const Info &info) -> std::string {                                        \
    return info.member ? ToStr(*info.member) : "";                             \
  }
#define SET(member)                                                            \
  [](std::string_view value, Info &info) { return FromSv(value, info.member); }
...
template <typename Info, typename Field> struct FieldMeta {
  std::string_view key;
...
Info GetPairedInfo(std::istream &is,
                   const std::vector<FieldMeta<Info, Field>> &table,
                   const EnumBitset<Field> &field_mask) {
  Info info;
  std::string line_buf;
...
static const std::vector<FieldMeta<Info, Field>> TABLE{
    {"VmPeak", ACCESS(vm_peak)},     {"VmSize", ACCESS(vm_size)},
    {"VmLck", ACCESS(vm_lck)},       {"VmPin", ACCESS(vm_pin)},
    {"VmHWM", ACCESS(vm_hwm)},       {"VmRSS", ACCESS(vm_rss)},
    {"RssAnon", ACCESS(rss_anon)},   {"RssFile", ACCESS(rss_file)},
    {"RssShmem", ACCESS(rss_shmem)}, {"VmData", ACCESS(vm_data)},
    {"VmStk", ACCESS(vm_stk)},       {"VmExe", ACCESS(vm_exe)},
    {"VmLib", ACCESS(vm_lib)},       {"VmPTE", ACCESS(vm_pte)},
