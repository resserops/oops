...
template <typename Iter> void StrSplitToIter(const std::string &str, Iter iter);
template <typename Iter>
void StrSplitToIter(const std::string &str, const std::string &delim,
                    Iter iter);
template <typename Iter>
void StrSplitToIter(const std::string &str, const std::string &delim,
                    bool skip_empty, Iter iter);

template <typename Iter>
void StrSplitToIterMultiDelim(const std::string &str, const std::string &delims,
                              bool skip_empty, Iter iter);
template <typename Iter>
void StrSplitToIterMultiDelim(const std::string &str, const std::string &delims,
                              Iter iter);
...
[[nodiscard]] std::string_view Strip(std::string_view sv,
                                     std::string_view chars = WHITE_SPACE);

template <typename T> T FromStr(const std::string_view sv);
} // namespace str
} // namespace oops
