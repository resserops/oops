...
namespace oops {
template <typename F> class FFloatPoint {
...
public:
  ...
  template <typename... Args> FTable &AppendRow(const Args &...args) {
    table_.back() = {ToStr(args)...};
    table_.emplace_back();
    return *this;
  }
  template <typename T> FTable &Append(const T &t, bool end = false) {
...
template <typename T, typename = void> struct HasOutput : ::std::false_type {};
template <typename T>
struct HasOutput<T, ::std::void_t<decltype(::std::declval<T>().Output(
                        ::std::declval<::std::ostream &>()))>>
    : ::std::true_type {};
template <typename T>
typename ::std::enable_if<HasOutput<T>::value, ::std::ostream &>::type
